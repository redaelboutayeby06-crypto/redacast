...
const VOICES = [
  { id: 'af_heart', label: 'Heart', desc: 'Warm & natural — best for Reddit stories', emoji: '❤️' },
    ...
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 40px', borderBottom: '1px solid #1a1a1a' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          <button onClick={signOut} style={{ background: 'transparent', border: '1px solid #222', borderRadius: '8px', padding: '7px 14px', color: '#666', cursor: 'pointer', fontSize: '13px' }}>Sign out</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '720px', margin: '50px auto', padding: '0 20px' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <p style={{ color: '#555', fontSize: '15px' }}>Type anything and get a professional voiceover in seconds</p>
        </div>
        ...
        <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '16px 20px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
            ...
            <span style={{ color: charsLeft < 500 ? '#ff6666' : '#00e5ff', fontSize: '13px', fontWeight: '600' }}>{charsLeft.toLocaleString()} / {WEEKLY_LIMIT.toLocaleString()} chars left</span>
          </div>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
            {VOICES.map(v => (
              <button key={v.id} onClick={() => setVoice(v.id)} style={{ background: voice === v.id ? 'rgba(0,229,255,0.08)' : '#0d0d0d', border: `1px solid ${voice === v.id ? 'rgba(0,229,255,0.4)' : '#1a1a1a'}`, borderRadius: '10px', padding: '12px 8px', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s ease' }}>
                ...
                <div style={{ color: '#444', fontSize: '10px', marginTop: '2px', lineHeight: '1.3' }}>{v.desc}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '20px', marginBottom: '16px' }}>
          <label style={{ color: '#555', fontSize: '13px', display: 'block', marginBottom: '10px' }}>Your script</label>
          <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Hey everyone, welcome back to my channel! Today we have an incredible story..." rows={6} style={{ width: '100%', background: '#111', border: '1px solid #222', borderRadius: '8px', padding: '14px', color: '#fff', fontSize: '15px', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'sans-serif', lineHeight: '1.6', outline: 'none' }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
            <span style={{ color: '#333', fontSize: '12px' }}>{selectedVoice?.emoji} {selectedVoice?.label} · {speed.toFixed(1)}x</span>
            <span style={{ color: text.length > charsLeft ? '#ff6666' : '#333', fontSize: '12px' }}>{text.length} chars</span>
          </div>
        </div>
